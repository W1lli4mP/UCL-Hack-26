...
def render_household_details_view(property_data: dict):
    ...
    with price_col3:
        ...
        if future_price:
            st.metric("Predicted Future Price", f"£{future_price:,.0f}")
        else:
            st.metric("Predicted Future Price", "Coming Soon")
    ...
    if postcode:
        try:
            ...
            historical_data = get_valuation_frame(area_code=postcode)
            
            # Placeholder for forecast and market data
            forecast_data = pd.DataFrame(columns=["date", "price"])
            market_data = pd.DataFrame(columns=["date", "price"])
            
            if historical_data is not None and not historical_data.empty:
                # Use the household_info_page function if available
                display_price_forecast_from_dataframes(historical_data, forecast_data, market_data)
                
                # Display price statistics
                display_price_statistics(historical_data, property_data)
            else:
                st.warning("No historical price data available for this postcode")
        except Exception as e:
            st.error(f"Error loading price history: {str(e)}")
    else:
        st.warning("Postcode required to display price history")
    ...
    with col1:
        if st.button("❤️ Save to Favorites", use_container_width=True):
            add_to_favorites(property_data)
    ...
    with st.expander("📋 View Raw Property Data"):
        st.json({
            ...
            "Current Valuation": property_data.get('current_price'),
            "Future Prediction": property_data.get('future_price'),
            "Last Sold Price": property_data.get('last_sold_price'),
            "Last Sold Date": property_data.get('last_sold_date'),
            "Sustainability Score": property_data.get('score'),
        })


def display_price_statistics(historical_data: pd.DataFrame, property_data: dict):
    """
    Display price statistics and trends.
